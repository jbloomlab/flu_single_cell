...
%DIF DEL eLife_original_submission.tex   Sun Jan 14 18:09:58 2018
%DIF ADD paper.tex                       Tue Jan 16 14:03:06 2018
...
We therefore engineered an influenza virus (strain A/WSN/1933) that additionally carried \emph{viral barcodes} consisting of synonymous mutations near the 3' end of each transcript (\FIG{workflow}A).
Critically, these synonymous mutations did not greatly impact viral growth kinetics (\FIG{workflow}B).
We infected A549 human lung carcinoma cells with an equal mix of the wild-type and synonymously barcoded viruses.
Cells infected by a single virion will exclusively express mRNAs from either wild-type or synonymously barcoded virus, whereas cells that are co-infected with multiple virions will often express mRNAs from both the wild-type and synonymously barcoded viruses (\FIG{workflow}C).
...
We validated that our stocks exhibited greater purity of infectious particles than a stock propagated at high MOI by verifying that they had a higher ratio of infectious particles to virion RNA (\FIG{viruspopulations}A) and to particles capable of inducing expression of a single viral protein (\FIG{viruspopulations}B).
In addition, viral stocks with many defective particles are more immunostimulatory \DIFdelbegin \DIFdel{~\mbox{%DIFAUXCMD
\citep{Tapia:2013kf,Lopez:2014en}}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{per infectious unit (e.g., TCID50) than low-defective stocks~\mbox{%DIFAUXCMD
\citep{Tapia:2013kf,Lopez:2014en}}%DIFAUXCMD
, in part simply because there are more physical virions per infectious unit (}\FIG{viruspopulations}\DIFadd{A,B)}\DIFaddend .
We confirmed that our viral stocks induced less interferon \DIFaddbegin \DIFadd{per infectious unit }\DIFaddend than a stock propagated at higher MOI (\FIG{viruspopulations}C).
...
{\bf (C)} 
Our viral stocks are less immunostimulatory than virus propagated at high MOI \DIFaddbeginFL \DIFaddFL{when used at the same number of infectious units as calculated by TCID50}\DIFaddendFL .
\DIFaddbeginFL \DIFaddFL{Note that this fact does not necessarily imply that they are more immunostimulatory per virion, as the high-MOI stocks also have more virions per infectious unit as shown in the first two panels. 
}\DIFaddendFL Measurements of \textit{IFNB1} transcript by qPCR normalized to the housekeeping gene L32 in A549 cells at 10 hours post infection at an MOI of 0.5.
Error bars $\pm$ S.D., n=3.
...
Full flow cytometry data for  \FIG{viruspopulations}B. 
...
We infected A549 cells at low MOI with a mixture of the wild-type and synonymously barcoded viruses, and collected cells for sequencing at 6, 8, and 10 hours post-infection, \DIFdelbegin \DIFdel{including a replicate a 8-hours.
We }\DIFdelend \DIFaddbegin \DIFadd{performing two slightly different variants of the experiment for the 8-hour timepoint.
For most of the samples, we }\DIFaddend replaced the infection inoculum with fresh media at one-hour post-infection, thereby ensuring that most infection was initiated during a narrow time window.
\DIFdelbegin \DIFdel{The exception was the replicate }\DIFdelend \DIFaddbegin \DIFadd{However, for the second }\DIFaddend 8-hour sample \DIFdelbegin \DIFdel{, for which we }\DIFdelend \DIFaddbegin \DIFadd{(which we denote as ``8hr-2'' in the figures), we }\DIFaddend did \emph{not} perform this media change and instead left the cells in the original infection inoculum.
\DIFaddbegin \DIFadd{The rationale for including a sample without a media change was to determine the importance of synchronicity of the timing of infection as discussed later in this subsection.
}

\DIFaddend We recovered between 3,000 and 4,000 cells for each sample (\FIG{cells}A). 
As expected for a low-MOI infection, most cells expressed little or no viral mRNA (\FIG{cells}B\DIFaddbegin \DIFadd{, }\FIGSUPP[cells]{viral_mRNA_cumulfrac}\DIFaddend ).
Also as expected, the amount of viral mRNA per cell among infected cells increased over time (\FIG{cells}B\DIFaddbegin \DIFadd{, }\FIGSUPP[cells]{viral_mRNA_cumulfrac}\DIFaddend ).
But what was most notable was how widely the number of viral mRNA molecules varied among infected cells.
While the fraction of mRNA derived from virus was $<$0.1\% for most cells, viral mRNA constituted half the transcriptome in a few cells at 8 and 10 hours (\FIG{cells}B\DIFaddbegin \DIFadd{, }\FIGSUPP[cells]{viral_mRNA_cumulfrac}\DIFaddend ).
...
\DIFaddbeginFL \FIGSUPP[cells]{viral_mRNA_cumulfrac} \DIFaddFL{shows the exact distributions of the fraction of viral mRNA per cell.
}\DIFaddendFL }
\figsupp[\DIFaddFL{Cumulative fraction plot of proportion of total mRNA from virus.} \label{figsupp:viral_mRNA_cumulfrac}]{
\DIFaddFL{  For each sample, this plot shows the fraction of all cells that derive at least the indication fraction of their mRNA from influenza virus.}
...
\FIG{viralbarcodes}A shows the fraction of viral reads in individual cells from each viral variant, and \FIG{viralbarcodes}B indicates the fraction of viral reads from the most abundant variant in that cell.
...
\begin{figure}[t!]
\DIFdelbeginFL %DIFDELCMD < \centerline{\includegraphics[width=0.9\linewidth]{figures/p_frac_flu_summary.pdf}}
%DIFDELCMD < %%%
...
We fit a curve \DIFaddbeginFL \DIFaddFL{(orange line) }\DIFaddendFL to the mean purity of all cells with more than the indicated amount of viral mRNA, and drew the cutoff \DIFaddbeginFL \DIFaddFL{(dotted green line) }\DIFaddendFL at the point where this curve stopped increasing with the fraction of total mRNA derived from virus.
\DIFaddbeginFL \DIFaddFL{This plot illustrates the process for the 10-hour sample, see }\FIGSUPP[viralbarcodes]{thresholds} \DIFaddFL{for similar plots for other samples.
}\DIFaddendFL See the Methods for details.
{\bf (D)}
...
\DIFaddbeginFL \FIGSUPP[viralbarcodes]{cumulfracflu} \DIFaddFL{shows these same data in a cumulative fraction plots and calculates Gini coefficients to quantify the heterogeneity in viral mRNA load.
}\DIFaddendFL }
\DIFaddbeginFL 

\DIFaddendFL \figsupp[Number of viral barcodes called.\label{figsupp:barcodescalled}]{
...
\DIFdelbeginFL %DIFDELCMD < \figsupp[Fraction of total viral mRNA derived from a given fraction of infected cells.%DIFDELCMD < \label{figsupp:cumulfracflu}%%%
]{
%DIFDELCMD < The total fraction of all viral mRNA among infected cells that is attributable to a given fraction of these cells.
%DIFDELCMD < For instance, the plot for the 8-hour sample shows that roughly 50\% of all viral mRNA is derived from 10\% of the infected cells.
%DIFDELCMD < }{\includegraphics[width=\linewidth]{figures/p_cumul_flu.pdf}}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL 

\figsupp[\DIFaddFL{Thresholds for calling infected cells.} \label{figsupp:thresholds}]{
\DIFaddFL{Cell lysis can lead cells to the spurious association of small amounts of extraneous mRNA with individual cells.
...
As is shown in} \FIG{viralbarcodes}\DIFaddFL{B, the 8hr-2 and 10hr sample clearly have an enrichment of mixed barcodes in cells with small numbers of viral mRNA.
...
For the 6hr and 8hr samples there is no indication of contamination from lysis-derived reads, as} \FIG{viralbarcodes}\DIFaddFL{B shows no increase in mixed barcodes in low viral mRNA cells.
Therefore, for these samples we simply set a threshold of requiring at least 2$\times$10$^{-4}$ of the total mRNA to come from virus, which corresponds to $\sim$2 viral mRNAs for the typical cell with $10^4$ total reads (}\FIG{cells}\DIFaddFL{B).}
}{\includegraphics[width=\linewidth]{figures/p_purity_threshold.pdf}}

\figsupp[Cumulative distributions of viral mRNA per cell \DIFaddFL{and Gini coefficients}.\label{figsupp:cumulfracflu}]{
The total fraction of all viral mRNA among infected cells that is attributable to a given fraction of these cells.
For instance, the plot for the 8hrs sample shows that $\sim$50\% of all viral mRNA is derived from $\sim$8\% of the infected cells.
\DIFaddFL{The facet titles above each plot also give the Gini coefficient}~\citep{gini1921measurement} \DIFaddFL{that calculates the heterogeneity in the distribution of viral mRNA among infected cells. Gini coefficients of 0 indicate a perfectly even distribution across cells, and Gini coefficients of 1 indicate a maximally skewed distribution.}
}{\includegraphics[width=\linewidth]{figures/p_cumul_flu.pdf}}

\figsupp[\DIFaddFL{Synchronization of infection does not greatly affect heterogeneity.} \label{figsupp:synchronize}]
{\DIFaddFL{Flow cytometry analysis of expression of viral proteins in cells infected at an MOI of 0.1 (unsynchronized) or 0.2 (synchronized) as calculated by TCID50 on MDCK-SIAT1 cells. 
The higher MOI for synchronized infections was to attempt to account for loss of virus in washing steps.
Infections were synchronized by pre-adsorbing virus at 4$^{\circ}$C for 1h prior to initiation of infection by shifting temperature to 37$^{\circ}$C using pre-warmed media.
Cells were concurrently stained for HA and NS1 proteins at 10 hours after initiation of infection, and then analyzed by flow cytometry.
The level of HA protein was quantified in cells that were identified as infected based on being positive for NS1 protein (top), and the level of NS1 protein was quantified in cells that were identified as infected based on being positive for HA protein (bottom)
Synchronization resulted in a small decrease in variability in the levels of each viral protein, particularly in cells with intermediate levels.
But the effects were small compared to the overall variability in the protein levels, indicating timing of infection makes only a small contribution to the observed heterogeneity. 
Data are shown for three independent replicates.
}}{\includegraphics[width=\linewidth]{figures/synchronize.jpg}}

\figsupp[Effects of infectious dose or coinfection state. \label{figsupp:flowdose}]
{
{\bf \DIFaddFL{(A)}}
\DIFaddFL{Flow cytometry analysis of expression of viral proteins in cells infected at high (MOI 5 as calculated by TCID50 on MDCK-SIAT1 cells) or low (MOI 0.1) initial infectious dose.
Cells were concurrently stained for HA and NS1 proteins at 10 hours after initiation of infection, and then analyzed by flow cytometry.
The level of HA protein was quantified in cells that were identified as infected based on being positive for NS1 protein (left), and the level of NS1 protein was quantified in cells that were identified as infected based on being positive for HA protein (right)
...
\DIFaddFL{Cells were co-infected with a mix of wild-type virus and pseudovirus in which the HA gene was replaced by GFP flanked by the terminal regions of the HA gene segment at an MOI of 0.1 for each virus. 
At 10 hours post-infection, cells were stained for NS1 and HA expression and analyzed by flow cytometry for these proteins and GFP.
Cells could be annotated as infected by virions of the same type (wild-type infection indicated by presence of HA, or pseudovirus infection indicated by the presence of GFP) or both types of virions (indicated by presence of HA and GFP).
...
We determined the threshold amount of viral mRNA per cell \DIFaddbegin \DIFadd{for each sample }\DIFaddend at which the barcode partitioning clearly resulted from infection rather than leakage (\FIG{viralbarcodes}C\DIFaddbegin \DIFadd{, }\FIGSUPP[viralbarcodes]{thresholds}\DIFaddend ), and used \DIFdelbegin \DIFdel{this threshold }\DIFdelend \DIFaddbegin \DIFadd{these thresholds }\DIFaddend to annotate cells that we were confident were truly infected.
We also annotated as co-infected cells above this threshold that had mRNA from both viral variants.
\FIG{viralbarcodes}D shows the number of cells annotated as infected and co-infected for each sample -- these cells are just a small fraction of the number of cells with any viral read.
These annotation thresholds are conservative, and \DIFdelbegin \DIFdel{will }\DIFdelend \DIFaddbegin \DIFadd{may }\DIFaddend miss some true low-level infections\DIFdelbegin \DIFdel{, as well as any co-infections with the same viral variant}\DIFdelend .
However, it is important that the analyses below are restricted to cells that are truly infected with virus, so we accepted the \DIFaddbegin \DIFadd{possible }\DIFaddend loss of some low-level infections in order to avoid false positives.
\DIFaddbegin \DIFadd{In addition, the synonymous viral barcodes only identify co-infections by viruses with different barcodes -- since the barcodes are at roughly equal proportion, we expect to miss about half of the co-infections.
Since we annotate about $\sim$10\% of the infected cells as co-infected by viruses with different barcodes (}\FIG{viralbarcodes}\DIFadd{D), we expect another $\sim$10\% of the infected cells to also be co-infected but not annotated as so by our approach.
}\DIFaddend Because most cells are not infected, we subsampled the uninfected cells to the numbers shown in \FIG{viralbarcodes}D to balance the proportions of infected and uninfected cells for all subsequent analyses.

Strikingly, the extreme variation in the number of viral transcripts per cell remains even after we apply these rigorous criteria for annotating infected cells (\FIG{viralbarcodes}E). 
...
At 6 and 8 hours $<$10\% of infected cells are responsible for over half the viral transcripts, while at 10 hours \DIFdelbegin \DIFdel{$\approx$}\DIFdelend \DIFaddbegin \DIFadd{$<$}\DIFaddend 15\% of infected cells produce over half the viral transcripts (\DIFdelbegin %DIFDELCMD < \FIGSUPP[viralbarcodes}]{cumulfracflu}%%%
\DIFdel{).
}\DIFdelend \DIFaddbegin \FIGSUPP[viralbarcodes]{cumulfracflu}\DIFadd{).
One way to quantify the heterogeneity of a distribution is to calculate the Gini coefficient~\mbox{%DIFAUXCMD
\citep{gini1921measurement}}%DIFAUXCMD
, which ranges from 0 for a completely uniform distribution, to 1 for a maximally skewed distribution.
}\FIGSUPP[viralbarcodes]{cumulfracflu} \DIFadd{shows the Gini coefficients for the distribution of viral mRNA across infected cells for each sample.
The Gini coefficients are $\ge$0.64 for all samples.
As a fun point of comparison, these Gini coefficients indicate that the distribution of viral mRNA across infected cells is more uneven than the distribution of income in the United States~\mbox{%DIFAUXCMD
\citep{alvaredo2011note}}%DIFAUXCMD
. 
}

\DIFadd{One possible source of heterogeneity in the amount of viral mRNA per cell is variability in the timing of infection.
...
First, the sample for which the infection inoculum was never removed (8hr-2) only shows slightly more heterogeneity than samples for which the inoculum was washed away after one hour (}\FIG{viralbarcodes}\DIFadd{E, }\FIGSUPP[viralbarcodes]{cumulfracflu}\DIFadd{), despite the fact that the potential time window for infection is much longer in the former sample.
Second, in an independent experiment, we performed completely synchronized infections by pre-binding virus to cells on ice and then washing away unbound virus before bringing the cells to 37$^{\circ}$C~\mbox{%DIFAUXCMD
\citep{Dapat:2014kv}}%DIFAUXCMD
.
As shown in }\FIGSUPP[viralbarcodes]{synchronize}\DIFadd{, flow cytometry staining found that the heterogeneity in the levels of individual viral proteins was not markedly different for these synchronized infections than in the absence of pre-binding and washing.
Finally, viral mRNA expression from the secondary spread of virus from infected cells does not appreciably occur during the timeframes of our experiments, since }\FIG{viralbarcodes}\DIFadd{B does not show the pervasive presence of mixed barcodes that would occur in this case.
...
\DIFaddend Notably, \DIFdelbegin \DIFdel{co-infected cells do not express dramatically more viral mRNA than other cells(}\DIFdelend \FIG{viralbarcodes}\DIFdelbegin \DIFdel{), }\DIFdelend \DIFaddbegin \DIFadd{E shows that there are co-infected cells with both low and high amounts of viral mRNA, }\DIFaddend suggesting that the initial infectious dose \DIFdelbegin \DIFdel{is not the major driver of cell-to-cell variation in our low-MOI infections.
}\DIFdelend \DIFaddbegin \DIFadd{does not drive a simple continuous increase in viral transcript production.
In support of this view, we used flow cytometry to quantify the levels of individual viral proteins in cells infected at various MOIs or for which we could delineate co-infection status (}\FIGSUPP[viralbarcodes]{flowdose}\DIFadd{).
This analysis shows that sub-populations of cells that express similarly low and high levels of viral proteins persist across a wide range of infectious doses, although co-infection can influence the relative proportion of infected cells that fall into these sub-populations (}\FIGSUPP[viralbarcodes]{flowdose}\DIFadd{).
}\DIFaddend 
...
This secondary transcription is a major source of viral mRNAs, as evidenced by the fact that blocking synthesis of the RNP proteins reduces the amount of viral mRNA by several orders of magnitude in bulk cells (\FIGSUPP[fluburdenbyflugene]{cyclohexamide}).

\begin{figure}[t!]
\DIFdelbeginFL %DIFDELCMD < \centerline{\includegraphics[width=0.9\linewidth]{figures/p_flu_burden_flu_gene_merge.pdf}}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \centerline{\includegraphics[width=0.83\linewidth]{figures/p_flu_burden_flu_gene_merge.pdf}}
\DIFaddendFL \caption{\label{fig:fluburdenbyflugene}
The absence of viral genes explains some of the variability in the amount of viral mRNA per cell.
{\bf (A)} 
\DIFdelbeginFL \DIFdelFL{Fraction }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{The normalized expression }\DIFaddendFL of \DIFdelbeginFL \DIFdelFL{mRNA in }\DIFdelendFL each \DIFdelbeginFL \DIFdelFL{infected cell derived from virus }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{viral gene }\DIFaddendFL as a function of the \DIFdelbeginFL \DIFdelFL{normalized expression }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{total fraction }\DIFaddendFL of \DIFdelbeginFL \DIFdelFL{each viral gene }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{mRNA }\DIFaddendFL in \DIFdelbeginFL \DIFdelFL{that }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{each infected }\DIFaddendFL cell \DIFaddbeginFL \DIFaddFL{derived from virus}\DIFaddendFL , taken over all time points.
...
\DIFaddbeginFL \DIFaddFL{Plots for individual samples are in }\FIGSUPP[fluburdenbyflugene]{fluburdensamplebreakdown}\DIFaddFL{, and a plot that excludes known coinfected cells is in }\FIGSUPP[fluburdenbyflugene]{fluburden_nocoinfection}\DIFaddFL{.
}\DIFaddendFL {\bf (B)}
...
\DIFaddbeginFL \DIFaddFL{The trends are similar if we look only at the 10-hour sample (}\FIGSUPP[fluburdenbyflugene]{10hrfluburdenbyflugene}\DIFaddFL{) or exclude known co-infected cells (}\FIGSUPP[fluburdenbyflugene]{fluburdenbyflugene_nocoinfection}\DIFaddFL{).
}\DIFaddendFL {\bf (C)}
...
\DIFaddendFL \figsupp[Secondary transcription is a major source of viral mRNA during bulk infections.\label{figsupp:cyclohexamide}]{
...
\DIFdelbeginFL %DIFDELCMD < \figsupp[Like panel (A), but shows samples individually.%DIFDELCMD < \label{figsupp:fluburdensamplebreakdown}%%%
]{
%DIFDELCMD < The fraction of mRNA in each infected cell derived as a function of the normalized expression of each viral gene, shown for the 10-hour and 8-hour samples individually (the other samples had too few infected cells for this analysis to be useful).
%DIFDELCMD < Points are colored as in \FIG{fluburdenbyflugene}A.
%DIFDELCMD < }{\includegraphics[width=0.8\linewidth]{figures/p_flu_burden_flu_gene_by_sample.pdf}}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL 
...
Points are colored as in \FIG{fluburdenbyflugene}A.
}{\includegraphics[width=0.8\linewidth]{figures/p_flu_burden_flu_gene_by_sample.pdf}}

\figsupp[\DIFaddFL{Like panel (A), but excludes coinfected cells with mixed viral barcodes.}\label{figsupp:fluburden_nocoinfection}]{
\DIFaddFL{The normalized expression of each viral gene as a function of the fraction of total mRNA derived from virus, excluding cells that were annotated as coinfected based on the presence of both viral barcodes.}
}{\includegraphics[width=0.8\linewidth]{figures/p_flu_burden_flu_gene_no_coinfect.pdf}}

\DIFaddendFL \figsupp[Like panel (B) but for the 10-hr sample only.\label{figsupp:10hrfluburdenbyflugene}]{
...
\DIFdelbeginFL %DIFDELCMD < \figdata{The numerical data for panel (C) are in \texttt{p\_missing\_genes.csv}.}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL 

\figsupp[\DIFaddFL{Like panel (B) but excludes coinfected cells with mixed viral barcodes.}\label{figsupp:fluburdenbyflugene_nocoinfection}]{
\DIFaddFL{All findings in} \FIG{fluburdenbyflugene}B \DIFaddFL{remain unchanged if we exclude cells called as coinfected based on the presence of mixed viral barcodes.}
...
\figdata{\label{figdata:simplemodel} \DIFaddFL{Simulation with a simple model for the expected heterogeneity due to Poisson co-infection and presence / absence of the full RNP is in \texttt{simple\_Poisson\_model.html}.}}

\DIFaddendFL} \end{figure}

We examined the total amount of viral mRNA versus the expression of \DIFdelbegin \DIFdel{each viral gene }\DIFdelend \DIFaddbegin \DIFadd{the genes from each viral segment }\DIFaddend (\FIG{fluburdenbyflugene}A, \FIGSUPP[fluburdenbyflugene]{fluburdensamplebreakdown}\DIFdelbegin \DIFdel{). 
}\DIFdelend \DIFaddbegin \DIFadd{, }\FIGSUPP[fluburdenbyflugene]{fluburden_nocoinfection}\DIFadd{). 
Note that influenza virus expresses ten major gene transcripts from its eight gene segments, as the M and NS segments are alternatively spliced to produce the M1 / M2 and NS1 / NEP transcript, respectively~\mbox{%DIFAUXCMD
\citep{dubois2014influenza}}%DIFAUXCMD
.
However, an inherent limitation of current established single-cell mRNA sequencing techniques is that they only sequence the 3' end of the transcript~\mbox{%DIFAUXCMD
\citep{zheng2017massively,macosko2015highly,Klein:2015ki,cao2017comprehensive}}%DIFAUXCMD
.
...
\DIFaddend Cells that lack an RNP gene never derive more than a few percent of their mRNAs from virus, confirming the expected result that all four RNP genes are essential for high levels of viral transcription \DIFaddbegin \DIFadd{(}\FIG{fluburdenbyflugene}\DIFadd{A, }\FIGSUPP[fluburdenbyflugene]{fluburdensamplebreakdown}\DIFadd{, }\FIGSUPP[fluburdenbyflugene]{fluburden_nocoinfection}\DIFadd{)}\DIFaddend .
...
These results are statistically supported by \FIG{fluburdenbyflugene}B, which shows that absence of any RNP gene but \emph{not} any other viral gene is associated with reduced amounts of viral mRNA. 
...
Specifically, at both 8 and 10 hours, the amount of viral mRNA in individual cells expressing all eight viral genes still ranges from $<$1\% to $>$50\% (\FIG{fluburdenbyflugene}A, \FIGSUPP[fluburdenbyflugene]{fluburdensamplebreakdown}\DIFdelbegin \DIFdel{).   
}\DIFdelend \DIFaddbegin \DIFadd{, }\FIGSUPP[fluburdenbyflugene]{fluburden_nocoinfection}\DIFadd{).  
Furthermore, the actual distribution of viral mRNA per infected cell (}\FIG{viralbarcodes}\DIFadd{E) does not match the mostly bi-modal shape expected under a simple model where RNP gene absence and Poisson co-infection are the only factors (}\FIG{fluburdenbyflugene}\DIFadd{--source~data~\ref{figdata:simplemodel}), indicating that there are additional sources of variability beyond whether cells have full complement of RNP genes. 
}\DIFaddend 
...
At the 8- and 10-hour time points, between 5\% and 17\% of cells fail to express any one of the four non-RNP genes (\FIG{fluburdenbyflugene}C\DIFaddbegin \DIFadd{, }\FIG{fluburdenbyflugene}\DIFadd{-source~data~\ref{figdata:fracmissing}}\DIFaddend ).
The absence of a given gene appears to be an independent event, as the probability of observing all four non-RNP genes in a cell is well predicted by simply multiplying the probabilities of observing each gene individually (\FIG{fluburdenbyflugene}C \DIFaddbegin \DIFadd{and }\FIG{fluburdenbyflugene}\DIFadd{-source~data~\ref{figdata:fracmissing}}\DIFaddend ). 
If we extrapolate the frequencies at which cells lack non-RNP genes to the RNP genes, then we would predict that \DIFdelbegin \DIFdel{40-50}\DIFdelend \DIFaddbegin \DIFadd{35-50}\DIFaddend \% of infected cells express mRNAs from all eight genes.
...
In contrast to the extreme variability in the total viral mRNA per cell, the fraction of this \DIFaddbegin \DIFadd{viral }\DIFaddend mRNA derived from each gene is much more consistent across cells (\FIG{fluexpr}A).
Total viral mRNA varies by orders of magnitude, but the fraction from any given viral gene is fairly tightly clustered around the median value for all cells (\FIG{fluexpr}B).
...
The cell-to-cell consistency in the relative expression of different viral genes is even tighter if we limit the analysis only to cells that express all eight viral genes (\FIG{fluexpr}C,D).
...
\DIFdelbegin \DIFdel{It is also reminiscent of the observation that total cellular mRNA varies among single cells as a function of factors such as volume that affect all mRNA species concordantly~\mbox{%DIFAUXCMD
\citep{padovan2015single}}%DIFAUXCMD
.
}\DIFdelend 
...
Overall, we captured 10 such co-infected cells that had $>$5\% of their mRNA derived from virus (\FIG{coexpression}).
...
For instance, the cell with 11.2\% of its mRNA from virus in the upper right of \FIG{coexpression} expresses M only from the wildtype viral variant, and NP and HA only from the synonymously barcoded variant.
...
\DIFaddend Another observation from \FIG{coexpression} is that co-infected cells usually express roughly equal amounts of transcripts from each of the two viral variants.
...
We then co-infected cells with a mix of wildtype and $\Delta$HA-GFP virus and used flow cytometry to score cells for the presence of HA only (infection by wildtype virus), GFP only (infection by $\Delta$HA-GFP virus), or both (co-infection) as shown in \FIGSUPP[coexpression]{flowcyto}.
...
%DIF < In bulk cells, this interferon response is one of the most prominent transcriptional signatures of influenza-virus infection~\citep{Geiss:2002a}.
...
\begin{figure}
\DIFdelbeginFL %DIFDELCMD < \centerline{\includegraphics[width=0.9\linewidth]{figures/p_small_tsne_merge_manualarrow.pdf}}
%DIFDELCMD < %%%
...
As shown in \FIG{tsne}A,B, this approach effectively grouped cells by the amount of viral mRNA that they expressed.
Sample-to-sample variation was regressed away during the clustering, as cells did not obviously group by time-point, with expected exception that the uninfected and 6-hour samples had few cells in the region of the plot corresponding to large amounts of viral mRNA (\FIG{tsne}C).
...
Remarkably, only a single cell expressed detectable interferon (\FIG{tsne}D).
We also examined interferon-stimulated genes, which are induced by autocrine and paracrine interferon signaling.
\FIG{tsne}E shows expression of one such gene, IFIT1~\citep{Fensterl:2011fp}.
...
Notably, interferon and interferon-stimulated genes were also relatively ineffective at blocking viral transcription in the single cell in which they were potently induced, since $>$10\% of the mRNA in this cell was derived from virus (\FIG{tsne}A,B,D,E).
...
We therefore identified cells that expressed substantial amounts of viral mRNA but lacked the NS gene (\FIG{tsne}F).
...
But other cells that lacked NS expression still failed to induce a detectable interferon response, despite often having a substantial amount of their mRNA derived from virus (\FIG{tsne}).
...
%DIFDELCMD < \centerline{\includegraphics[width=0.7\linewidth]{figures/p_cellular_heatmap_manualedit.pdf}}
%DIFDELCMD < %%%
...
\DIFdelbeginFL %DIFDELCMD < \figdata{The full results of the differential expression test is in \texttt{p\_sig\_cellular\_genes.csv}.}
%DIFDELCMD < \figdata{The results of a gene-set analysis are in \texttt{p\_pathway\_enrichment.tsv}.}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL 

\figdata{The full results of the differential expression test are in \texttt{p\_sig\_cellular\_genes.csv}. \label{figdata:cellulargenes}}

\figdata{A gene-set analysis for pathways associated with the amount of viral mRNA is in \texttt{p\_pathway\_enrichment.csv}. \label{figdata:geneset}}

\DIFaddendFL \figsupp[Many genes that co-vary with viral load are involved in the oxidative stress response. \label{figsupp:ROStable}]{
Table delineating genes in \FIG{cellulargenes} that are associated with the response to oxidative stress~\citep{Duong:2017ju,Jung:2017jz,Lee:2017ba,Peuchant:2017hj,MacLeod:2016kt,Jiang:2016cf,Gorrini:2013jv,Miura:2013fr,Kim:2009gi,Banning:2005cz,Murray:2003ie,Doyle:1999ts}.
}{\includegraphics[width=0.7\linewidth]{figures/ROStable.pdf}}
\DIFaddbeginFL 

\figsupp[\DIFaddFL{Pre-treating to reduce oxidative stress decreases the fraction of infected cells expressing detectable viral protein.} \label{figsupp:oxidative}]
{\DIFaddFL{Cells were treated as indicated with 15 mM N-acetylcysteine (NAC), an antioxidant, and infected at an MOI of 0.1 as calculated by TCID50 on MDCK-SIAT1 cells. 
At 10 hours post-infection, cells were analyzed by flow cytometry for HA expression.
The percentage of HA-positive cells is indicated on the flow cytometry plots.
Data are shown for three independent replicates.
}}{\includegraphics[width=0.7\linewidth]{figures/antiox.jpg}}

\DIFaddendFL \end{figure}

\DIFdelbegin \DIFdel{Many of the genes with increased expression in cells with more }\DIFdelend \DIFaddbegin \DIFadd{A gene-set analysis shows that many cellular genes that are associated with the amount of }\DIFaddend viral mRNA are \DIFaddbegin \DIFadd{involved in the response to reactive oxygen species or hypoxia (}\FIG{cellulargenes}\DIFadd{-source~data~\ref{figdata:geneset}). 
Genes }\DIFaddend known or suspected to be regulated by the Nrf2 master regulator in response to oxidative stress \DIFaddbegin \DIFadd{are often expressed at higher levels in cells with more viral mRNA (}\FIG{cellulargenes}\DIFadd{)}\DIFaddend .
These genes produce proteins that are involved in detoxification of reactive oxygen species or resultant products, the management of misfolded proteins, the electron transport chain, or a general stress response (\FIGSUPP[cellulargenes]{ROStable}). 
...
\DIFdelbegin \DIFdel{Our data do not reveal whether the expression of genes involved in the response to oxidative stress are }\DIFdelend \DIFaddbegin \DIFadd{To directly test the effect of transient oxidative stress, we compared the fraction of cells that expressed detectable viral protein when infected either with or without pre-treatment to suppress oxidative stress.
}\FIGSUPP[cellulargenes]{oxidative} \DIFadd{shows that the cells pre-treated with an antioxidant exhibited less frequent detectable expression of viral protein.
These results, in conjunction with the differential expression test in }\FIG{cellulargenes} \DIFadd{and the prior work mentioned above, suggest that oxidative stress acts in a proviral fashion. 
}

\DIFadd{The gene-set analysis also found that the amount of viral mRNA was associated with the expression of genes involved in the G2-M cell-cycle checkpoint (}\FIG{cellulargenes}\DIFadd{-source~data~\ref{figdata:geneset}).
The cell-cycle associated genes CCND3, MKI67, UBE2S, and CENPF are all expressed at significantly lower levels in cells with more viral mRNA (}\FIG{cellulargenes}\DIFadd{).
However, our data are not sufficient to determine whether the lower expression of these genes is }\DIFaddend a cause or \DIFdelbegin \DIFdel{a symptom of higher levels of viral mRNA, and further investigation of this topic is an interesting area for future work.
}\DIFdelend \DIFaddbegin \DIFadd{effect of the reduction in viral mRNA.
}\DIFaddend 

\DIFaddbegin \DIFadd{Interestingly, none of the cellular genes that are significantly associated with the amount of viral mRNA in our study are among the 128 genes that \mbox{%DIFAUXCMD
\citet{watanabe2010cellular} }%DIFAUXCMD
report as having been identified multiple times in genome-wide screens for factors affecting influenza virus replication.
...
Overall, our work shows the power and importance of \DIFdelbegin \DIFdel{charactering }\DIFdelend \DIFaddbegin \DIFadd{characterizing }\DIFaddend cellular infection at the level of single cells~\citep{avraham2015pathogen}.
\DIFdelbegin \DIFdel{The dynamics of viral infection in any given cell is shaped by }\DIFdelend \DIFaddbegin \DIFadd{Viral infection can involve heterogeneity in }\DIFaddend the genetic composition of the incoming virion, the host-cell state, the bi-modality of innate-immune activation, and the inherent stochasticity of molecular processes initiated by a single copy of each viral gene.
\DIFdelbegin \DIFdel{We have shown how the confluence of these factors leads to extreme cell-to-cell }\DIFdelend \DIFaddbegin \DIFadd{In our experiments with short-timeframe and low-MOI infections with a relatively pure stock of influenza virus, we find only a minor role for innate-immune activation, but a substantial role for }\DIFaddend heterogeneity in the \DIFdelbegin \DIFdel{transcriptional outcome of influenza virus infection.
Further }\DIFdelend \DIFaddbegin \DIFadd{complement of viral genes that are expressed in individual cells and at least some contribution of host-cell state.
...
Future extensions of the approaches described here should enable further }\DIFaddend deconstruction of the \DIFdelbegin \DIFdel{contributions of each factor will enable }\DIFdelend \DIFaddbegin \DIFadd{sources of cell-to-cell heterogeneity during viral infection, thereby enabling }\DIFaddend a deeper understanding of how the bulk features of infection emerge from \DIFdelbegin \DIFdel{the processes occurring }\DIFdelend \DIFaddbegin \DIFadd{processes }\DIFaddend within individual virus-infected cells.
...
The sequences of the viral RNAs encoded in these plasmids are in \FIG{workflow}-source~data~\ref{figdata:viralsequences}.
...
The sequences of the synonymously barcoded viral RNAs are in \FIG{workflow}-source~data~\ref{figdata:viralsequences}.
...
The high-MOI passage (high-defective particle) stock used in \FIG{viruspopulations} was generated by instead passaging in MDCK-SIAT1 cells twice at an MOI of 1 for 48 hours.

For the experiments in \FIGSUPP[coexpression]{flowcyto}, we created a virus that carried an HA gene segment in which GFP replaced most of the HA coding sequence, following a scheme first described by \citet{marsh2007specific}.
Briefly, we created a plasmid encoding a viral RNA with GFP in place of the HA coding sequence in the context of the pHH21~\citep{Neumann:1999ws} reverse-genetics plasmid, removing potential start codons upstream of the GFP (see \FIG{coexpression}-source~data~\ref{figdata:HAflank} for the sequence of the viral RNA).
...
\subsection{qPCR}
For the qPCR in \FIG{viruspopulations} and \FIGSUPP[fluburdenbyflugene]{cyclohexamide}, A549 cells were seeded at 3$\times$10\textsuperscript{5} cells per well in a 6-well tissue culture plate in D10 the day prior to infection. 
...
For the measurements of viral genomic HA content in \FIG{viruspopulations}A, vRNA was harvested from 80 \si{\micro}l of viral supernatant by the addition of 600 \si{\micro}l of RLT plus before proceeding with the standard QIAGEN RNeasy Plus Mini kit protocol.
...
To determine viral titers in terms of HA-expressing units and for the flow cytometry\DIFdelbegin \DIFdel{in }%DIFDELCMD < \FIGSUPP[coexpression]{flowcyto}%%%
\DIFdelend , A549 cells were seeded in a 6-well plate and infected as described above for the qPCR analyses.
...
After washing in PBS \DIFdelbegin \DIFdel{plus }\DIFdelend \DIFaddbegin \DIFadd{supplemented with }\DIFaddend 2\% FBS, the cells were stained with a goat anti-mouse IgG antibody conjugated to APC.
...
After channels were compensated and cells gated to exclude multiplets and debris }\DIFaddend in FlowJo, data were extracted using the R package flowCore~\citep{LeMeur:2007uo} and analyzed using a custom Python script.
...
\DIFaddbegin \DIFadd{For gating on NS1 positive cells, the percentage of influenza-infected cells was determined by HA staining alone, and the top quantile of NS1-stained cells matching that percentage were taken as the NS1 positive population.
}\DIFaddend 
...
\DIFdelbegin \DIFdel{The deep sequencing data are available on the Sequence Read Archive under accession }%DIFDELCMD < \jdbcomment{Upload in progress.}
%DIFDELCMD < %%%
\DIFdelend 
...
The influenza virus transcriptome was generated from the reverse-complement of the wildtype WSN viral RNA sequences as encoded in the reverse-genetics plasmids (\FIG{workflow}-source~data~\ref{figdata:viralsequences}).
\DIFaddbegin \DIFadd{The aligned deep sequencing data are available on the GEO repository under accession GSE108041 (}\url{https://www.ncbi.nlm.nih.gov/geo/query/acc.cgi?acc=GSE108041}\DIFadd{).
}

\DIFaddend CellRanger calls cells based on the number of observed cell barcodes, and creates a cell-gene matrix.
...
Only about half of the viral reads overlapped the barcoded regions of the genes (\FIG{workflow}A) and could therefore be assigned to a viral barcode (\FIGSUPP[viralbarcodes]{barcodescalled}).
...
A Jupyter notebook that performs these analyses is in Supplementary~file~\ref{suppfile:code}.
For each sample, cell barcodes that had $>$2.5-fold fewer or more UMI counts mapping to cellular transcripts than the sample mean were excluded from downstream analyses (see red vertical lines in \FIG{cells}B).

In order to determine an appropriate cutoff for how many reads a cell needed to contain in order to be classified as infected, we calculated the mean viral barcode purity across all cells that contained at least a given fraction of viral mRNA and had multiple viral reads that could be assigned a barcode (\FIG{viralbarcodes}B,C \DIFaddbegin \DIFadd{and }\FIGSUPP[viralbarcodes]{thresholds}\DIFaddend ).
...
\DIFdelbegin \DIFdel{The }\DIFdelend \DIFaddbegin \DIFadd{As is apparent from }\FIG{viralbarcodes}\DIFadd{B, only the }\DIFaddend 10-hour sample \DIFdelbegin \DIFdel{has the most viral mRNA, and so is expected to }\DIFdelend \DIFaddbegin \DIFadd{and the 8hr-2 sample have the excess of mixed barcodes among cells with low amounts of viral mRNA.
The likely reason is that these samples have more total viral mRNA (and so there is more available mRNA to be acquired from lysed cells); in addition, there is always some experimental variability in the amount of cell lysis during the 10X sequencing process, and these samples may simply }\DIFaddend have the most\DIFdelbegin \DIFdel{lysis-associated influenza reads.
We therefore determined the threshold using just this sample, and then applied this threshold to all samples.
The threshold is }\DIFdelend \DIFaddbegin \DIFadd{.
So the above threshold procedure is appropriate for those two samples.
For the other samples, we simply set a minimum threshold of requiring at least a fraction 2$\times$$10^{-4}$ reads to come from viral mRNA as explained in the legend to }\FIGSUPP[viralbarcodes]{thresholds}\DIFadd{.
The thresholds for each sample are }\DIFaddend shown in \FIG{viralbarcodes}C \DIFaddbegin \DIFadd{and }\FIGSUPP[viralbarcodes]{thresholds}\DIFaddend .
...
The specificity of these markers was determined using the function calculateMarkerSpecificity -- the top \DIFdelbegin \DIFdel{40 }\DIFdelend \DIFaddbegin \DIFadd{50 }\DIFaddend markers were retained, and used to place populations in a two-dimensional plane based on tSNE dimensionality reduction.
...
\FIG{cellulargenes} shows all genes that were significantly associated with the fraction of mRNA from virus at a false discovery rate of 0.1.
\DIFaddbegin \DIFadd{We performed the gene set analysis using the P -alues from the Monocle differentialGeneTest with }\texttt{\DIFadd{piano}}\DIFadd{~\mbox{%DIFAUXCMD
\citep{varemo2013enriching} }%DIFAUXCMD
using the hallmark gene set from GSEA v6~\mbox{%DIFAUXCMD
\citep{subramanian2005gene} }%DIFAUXCMD
and Fisher's method.
}\DIFaddend 
...
\DIFaddbegin \DIFadd{To just run the Monocle analysis in }\texttt{\DIFadd{monocle\_analysis.ipynb}} \DIFadd{on a pre-generated cell-gene matrix, unpack Supplementary~file~\ref{suppfile:cellgenematrix} into }\texttt{\DIFadd{./results/cellgenecounts/}}\DIFadd{.}\DIFaddend }
\end{suppfile}

\begin{suppfile}
\caption{\label{suppfile:cellgenematrix}
The annotated cell-gene matrix in Matrix Market Format.
\DIFdelbegin %DIFDELCMD < \jdbcomment{This file is too large for the \textit{eLife} submission system. We will communicate with the editors to get it uploaded for a final accepted version, or we will post it on DataDryad. If you are a reviewer and need access, please contact the editor.}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{This is the matrix generated in }\texttt{\DIFadd{./results/cellgenecounts/}} \DIFadd{by running the CellRanger analysis in }\texttt{\DIFadd{align\_and\_annotate.ipynb}} \DIFadd{in Supplementary~file~\ref{suppfile:code}.
}\DIFaddend }
\end{suppfile}
