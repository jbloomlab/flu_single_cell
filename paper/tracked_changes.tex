...
%DIF ADD paper.tex                       Sun Jan 14 17:59:21 2018
...
\DIFaddbeginFL \figsupp[Cumulative fraction plot of proportion of total mRNA from virus. \label{figsupp:viral_mRNA_cumulfrac}]{
...
As is shown in \FIG{viralbarcodes}B, the 8hr-2 and 10hr sample clearly have an enrichment of mixed barcodes in cells with small numbers of viral mRNA.
...
For the 6hr and 8hr samples there is no indication of contamination from lysis-derived reads, as \FIG{viralbarcodes}B shows no increase in mixed barcodes in low viral mRNA cells.
Therefore, for these samples we simply set a threshold of requiring at least 2$\times$10$^{-4}$ of the total mRNA to come from virus, which corresponds to $\sim$2 viral mRNAs for the typical cell with $10^4$ total reads (\FIG{cells}B).
}{\includegraphics[width=\linewidth]{figures/p_purity_threshold.pdf}}

\figsupp[Cumulative distributions of viral mRNA per cell and Gini coefficients.\label{figsupp:cumulfracflu}]{
...
The facet titles above each plot also give the Gini coefficient~\citep{gini1921measurement} that calculates the heterogeneity in the distribution of viral mRNA among infected cells.
Gini coefficients of 0 indicate a perfectly even distribution across cells, and Gini coefficients of 1 indicate a maximally skewed distribution.
}{\includegraphics[width=\linewidth]{figures/p_cumul_flu.pdf}}

\figsupp[Synchronization of infection does not greatly affect heterogeneity. \label{figsupp:synchronize}]
{\jdbcomment{Alistair, can you add caption}
}{\jdbcomment{Alistair, can you add add figure}}
...
Second, in an independent experiment, we performed completely synchronized infections by pre-binding virus to cells on ice and then washing away unbound virus before bringing the cells to 37$^{\circ}$C~}\jdbcomment{Alistair, can you add relevant citation for this method?}\DIFadd{.
...
All findings in \FIG{fluburdenbyflugene}B remain unchanged if we exclude cells called as coinfected based on the presence of mixed viral barcodes.
...
\figdata{\label{figdata:simplemodel} Simulation with a simple model for the expected heterogeneity due to Poisson co-infection and presence / absence of the full RNP is in \texttt{simple\_Poisson\_model.html}.}
...
Furthermore, the actual distribution of viral mRNA per infected cell (}\FIG{viralbarcodes}\DIFadd{E) is does not match the mostly bi-modal shape expected under a simple model where RNP gene absence and Poisson co-infection are the only factors (}\FIG{fluburdenbyflugene}\DIFadd{--source~data~\ref{figdata:simplemodel}), indicating that there are additional sources of variability beyond whether cells have full complement of RNP genes. 
}\DIFaddend 
...
\begin{figure}
\DIFdelbeginFL %DIFDELCMD < \centerline{\includegraphics[width=0.9\linewidth]{figures/p_flu_expr_manualedits.pdf}}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \centerline{\includegraphics[width=0.9\linewidth]{figures/p_flu_expr.pdf}}
\DIFaddendFL \caption{\label{fig:fluexpr}
\DIFaddbeginFL \jdbcomment{Alistair, can you add expression wedges for panels A and C?}
\DIFaddendFL Relative expression of influenza virus genes in highly infected cells (>5\% of total mRNA from virus).
{\bf (A)} 
...
In contrast to the extreme variability in the total viral mRNA per cell, the fraction of this mRNA derived from each gene is much more consistent across cells (\FIG{fluexpr}A).
...
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \centerline{\includegraphics[width=0.9\linewidth]{figures/p_small_tsne_merge.pdf}}
\DIFaddendFL \caption{\label{fig:tsne}
\DIFaddbeginFL \jdbcomment{Alistair, can you add an arrow pointing to the IFN positive cell in panels D, E, and F.}
\DIFaddendFL A t-SNE plot created by semi-supervised clustering using genes that co-vary with viral infection status.
...
\DIFaddbeginFL \jdbcomment{Alistair, can you add a gray arrow wedge indicating expression increases from left to right.}
\DIFaddendFL Cellular genes that co-vary in expression with the amount of viral mRNA in cells expressing all eight viral genes.
...
%DIFDELCMD < \figsupp[Many genes that co-vary with viral load are involved in the oxidative stress response. %DIFDELCMD < \label{figsupp:ROStable}%%%
]{
%DIFDELCMD < Table delineating genes in \FIG{cellulargenes} that are associated with the response to oxidative stress~\citep{Duong:2017ju,Jung:2017jz,Lee:2017ba,Peuchant:2017hj,MacLeod:2016kt,Jiang:2016cf,Gorrini:2013jv,Miura:2013fr,Kim:2009gi,Banning:2005cz,Murray:2003ie,Doyle:1999ts}.
%DIFDELCMD < }{\includegraphics[width=0.7\linewidth]{figures/ROStable.pdf}}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL 
...
\jdbcomment{Alistair, can you remove GPX2, which is no longer significant.}
...
\figsupp[Pre-treating to induce oxidative stress increases the fraction of infected cells expressing detectable viral protein. \label{figsupp:oxidative}]
{
\jdbcomment{Alistair, can you add figure and caption showing the increase in the percent of flu positive cells after treatment. It should just be possible to put the methods for this experiment in the figure caption.}
}
{\jdbcomment{Alistair, and you add the figure.}}
...
\DIFdelbegin \DIFdel{Our data do not reveal whether the expression of genes involved in the response to oxidative stress are }\DIFdelend \DIFaddbegin \DIFadd{To directly test the effect of transient oxidative stress, we compared the fraction of cells that expressed detectable viral protein when infected either with or without pre-treatment to induce oxidative stress.
}\FIGSUPP[cellulargenes]{oxidative} \DIFadd{shows that the cells pre-treated to induce oxidative stress exhibited more frequent detectable expression of viral protein.
...
To determine viral titers in terms of HA-expressing units and for the flow cytometry in \FIGSUPP[coexpression]{flowcyto} \DIFaddbegin \DIFadd{and }\FIGSUPP[viralbarcodes]{flowdose}\DIFaddend , A549 cells were seeded in a 6-well plate and infected as described above for the qPCR analyses.
...
%DIFDELCMD < %%%
\DIFdel{For }\DIFdelend \DIFaddbegin \DIFadd{For NS1 staining in }\FIGSUPP[viralbarcodes]{flowdose}\DIFadd{, cells stained for HA as described above were permeabilized using BD Cytofix/Cytoperm following manufacturer's instructions, stained with anti-NS1 (GTX125990, Genetex) at 4.4 \si{\micro}g/ml, washed,  stained with a  goat anti-rabbit IgG antibody conjugated to Alexa Fluor 405. washed, and analyzed.
For }\DIFaddend \FIGSUPP[coexpression]{flowcyto} \DIFaddbegin \DIFadd{and }\FIGSUPP[viralbarcodes]{flowdose}\DIFaddend , after gating to exclude multiplets in FlowJo, data were extracted using the R package flowCore~\citep{LeMeur:2007uo} and analyzed using a custom Python script.
\DIFaddbegin \DIFadd{For }\FIGSUPP[viralbarcodes]{flowdose} \DIFadd{channels were additionally compensated in FLowJo.
}\DIFaddend Guassian kernel density estimates were obtained using the scipy stats package method, guassian\_kde, using automatic bandwidth determination~\citep{vanderWalt:2017dp}.
\DIFaddbegin \DIFadd{For }\FIGSUPP[viralbarcodes]{flowdose}\DIFadd{, the percentage of influenza-infected cells was determined by HA staining alone, and the top quantile of NS1-stained cells matching that percentage were taken as the NS1 positive population.
}\DIFaddend 
