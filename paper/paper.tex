...
In addition, viral stocks with many defective particles are more immunostimulatory per infectious unit (e.g., TCID50) than low-defective stocks~\citep{Tapia:2013kf,Lopez:2014en}, in part simply because there are more physical virions per infectious unit (\FIG{viruspopulations}A,B).
We confirmed that our viral stocks induced less interferon per infectious unit than a stock propagated at higher MOI (\FIG{viruspopulations}C).
...
We recovered between 3,000 and 4,000 cells for each sample (\FIG{cells}A). 
As expected for a low-MOI infection, most cells expressed little or no viral mRNA (\FIG{cells}B, \FIGSUPP[cells]{viral_mRNA_cumulfrac}).
Also as expected, the amount of viral mRNA per cell among infected cells increased over time (\FIG{cells}B, \FIGSUPP[cells]{viral_mRNA_cumulfrac}).
But what was most notable was how widely the number of viral mRNA molecules varied among infected cells.
While the fraction of mRNA derived from virus was $<$0.1\% for most cells, viral mRNA constituted half the transcriptome in a few cells at 8 and 10 hours (\FIG{cells}B, \FIGSUPP[cells]{viral_mRNA_cumulfrac}).
...
\FIGSUPP[cells]{viral_mRNA_cumulfrac} shows the exact distributions of the fraction of viral mRNA per cell.
}
...
This plot illustrates the process for the 10-hour sample, see \FIGSUPP[viralbarcodes]{thresholds} for similar plots for other samples.
...
\FIGSUPP[viralbarcodes]{cumulfracflu} shows these same data in a cumulative fraction plots and calculates Gini coefficients to quantify the heterogeneity in viral mRNA load.
}
...
{Flow cytometry analysis of expression of viral proteins in cells infected at an MOI of 0.1 (unsynchronized) or 0.2 (synchronized) as calculated by TCID50 on MDCK-SIAT1 cells. 
...
{
{\bf (A)}
Flow cytometry analysis of expression of viral proteins in cells infected at high (MOI 5 as calculated by TCID50 on MDCK-SIAT1 cells) or low (MOI 0.1) initial infectious dose.
Cells were concurrently stained for HA and NS1 proteins at 10 hours after initiation of infection, and then analyzed by flow cytometry.
...
{\bf (B)}
Cells were co-infected with a mix of wild-type virus and pseudovirus in which the HA gene was replaced by GFP flanked by the terminal regions of the HA gene segment at an MOI of 0.1 for each virus. 
...
We determined the threshold amount of viral mRNA per cell for each sample at which the barcode partitioning clearly resulted from infection rather than leakage (\FIG{viralbarcodes}C, \FIGSUPP[viralbarcodes]{thresholds}), and used these thresholds to annotate cells that we were confident were truly infected.
...
Since we annotate about $\sim$10\% of the infected cells as co-infected by viruses with different barcodes (\FIG{viralbarcodes}D), we expect another $\sim$10\% of the infected cells to also be co-infected but not annotated as so by our approach.
Because most cells are not infected, we subsampled the uninfected cells to the numbers shown in \FIG{viralbarcodes}D to balance the proportions of infected and uninfected cells for all subsequent analyses.
...
At 6 and 8 hours $<$10\% of infected cells are responsible for over half the viral transcripts, while at 10 hours $<$15\% of infected cells produce over half the viral transcripts (\FIGSUPP[viralbarcodes]{cumulfracflu}).
...
\FIGSUPP[viralbarcodes]{cumulfracflu} shows the Gini coefficients for the distribution of viral mRNA across infected cells for each sample.
...
As shown in \FIGSUPP[viralbarcodes]{synchronize}, flow cytometry staining found that the heterogeneity in the levels of individual viral proteins was not markedly different for these synchronized infections than in the absence of pre-binding and washing.
Finally, viral mRNA expression from the secondary spread of virus from infected cells does not appreciably occur during the timeframes of our experiments, since \FIG{viralbarcodes}B does not show the pervasive presence of mixed barcodes that would occur in this case.
...
Notably, \FIG{viralbarcodes}E shows that there are co-infected cells with both low and high amounts of viral mRNA, suggesting that the initial infectious dose does not drive a simple continuous increase in viral transcript production.
In support of this view, we used flow cytometry to quantify the levels of individual viral proteins in cells infected at various MOIs or for which we could delineate co-infection status (\FIGSUPP[viralbarcodes]{flowdose}).
This analysis shows that sub-populations of cells that express similarly low and high levels of viral proteins persist across a wide range of infectious doses, although co-infection can influence the relative proportion of infected cells that fall into these sub-populations (\FIGSUPP[viralbarcodes]{flowdose}).
...
Plots for individual samples are in \FIGSUPP[fluburdenbyflugene]{fluburdensamplebreakdown}, and a plot that excludes known coinfected cells is in \FIGSUPP[fluburdenbyflugene]{fluburden_nocoinfection}.
{\bf (B)}
...
The trends are similar if we look only at the 10-hour sample (\FIGSUPP[fluburdenbyflugene]{10hrfluburdenbyflugene}) or exclude known co-infected cells (\FIGSUPP[fluburdenbyflugene]{fluburdenbyflugene_nocoinfection}).
{\bf (C)}
...
We examined the total amount of viral mRNA versus the expression of the genes from each viral segment (\FIG{fluburdenbyflugene}A, \FIGSUPP[fluburdenbyflugene]{fluburdensamplebreakdown}, \FIGSUPP[fluburdenbyflugene]{fluburden_nocoinfection}). 
...
Cells that lack an RNP gene never derive more than a few percent of their mRNAs from virus, confirming the expected result that all four RNP genes are essential for high levels of viral transcription (\FIG{fluburdenbyflugene}A, \FIGSUPP[fluburdenbyflugene]{fluburdensamplebreakdown}, \FIGSUPP[fluburdenbyflugene]{fluburden_nocoinfection}).
...
Specifically, at both 8 and 10 hours, the amount of viral mRNA in individual cells expressing all eight viral genes still ranges from $<$1\% to $>$50\% (\FIG{fluburdenbyflugene}A, \FIGSUPP[fluburdenbyflugene]{fluburdensamplebreakdown}, \FIGSUPP[fluburdenbyflugene]{fluburden_nocoinfection}).  
Furthermore, the actual distribution of viral mRNA per infected cell (\FIG{viralbarcodes}E) does not match the mostly bi-modal shape expected under a simple model where RNP gene absence and Poisson co-infection are the only factors (\FIG{fluburdenbyflugene}--source~data~\ref{figdata:simplemodel}), indicating that there are additional sources of variability beyond whether cells have full complement of RNP genes. 
...
At the 8- and 10-hour time points, between 5\% and 17\% of cells fail to express any one of the four non-RNP genes (\FIG{fluburdenbyflugene}C, \FIG{fluburdenbyflugene}-source~data~\ref{figdata:fracmissing}).
The absence of a given gene appears to be an independent event, as the probability of observing all four non-RNP genes in a cell is well predicted by simply multiplying the probabilities of observing each gene individually (\FIG{fluburdenbyflugene}C and \FIG{fluburdenbyflugene}-source~data~\ref{figdata:fracmissing}). 
...
In contrast to the extreme variability in the total viral mRNA per cell, the fraction of this viral mRNA derived from each gene is much more consistent across cells (\FIG{fluexpr}A).
...
Another observation from \FIG{coexpression} is that co-infected cells usually express roughly equal amounts of transcripts from each of the two viral variants.
...
We identified 43 cellular genes that co-varied with viral mRNA expression at a false discovery rate of 0.1 (\FIG{cellulargenes}, \FIG{cellulargenes}-source~data~\ref{figdata:cellulargenes}).
...
\figsupp[Pre-treating to reduce oxidative stress decreases the fraction of infected cells expressing detectable viral protein. \label{figsupp:oxidative}]
{Cells were treated as indicated with 15 mM N-acetylcysteine (NAC), an antioxidant, and infected at an MOI of 0.1 as calculated by TCID50 on MDCK-SIAT1 cells. 
...
A gene-set analysis shows that many cellular genes that are associated with the amount of viral mRNA are involved in the response to reactive oxygen species or hypoxia (\FIG{cellulargenes}-source~data~\ref{figdata:geneset}). 
Genes known or suspected to be regulated by the Nrf2 master regulator in response to oxidative stress are often expressed at higher levels in cells with more viral mRNA (\FIG{cellulargenes}).
...
\FIGSUPP[cellulargenes]{oxidative} shows that the cells pre-treated with an antioxidant exhibited less frequent detectable expression of viral protein.
These results, in conjunction with the differential expression test in \FIG{cellulargenes} and the prior work mentioned above, suggest that oxidative stress acts in a proviral fashion. 

The gene-set analysis also found that the amount of viral mRNA was associated with the expression of genes involved in the G2-M cell-cycle checkpoint (\FIG{cellulargenes}-source~data~\ref{figdata:geneset}).
The cell-cycle associated genes CCND3, MKI67, UBE2S, and CENPF are all expressed at significantly lower levels in cells with more viral mRNA (\FIG{cellulargenes}).
...
The A549 cells were tested as negative for mycoplasma contamination by the Fred Hutch Genomics Core.
...
Jupyter notebooks that perform all of the computational analyses are available in Supplementary~file~\ref{suppfile:code} and at \url{https://github.com/jbloomlab/flu_single_cell}~\citep{russell2018github}. \jdbcomment{This GitHub repository will be made public upon acceptance of the manuscript.}
...
As is apparent from \FIG{viralbarcodes}B, only the 10-hour sample and the 8hr-2 sample have the excess of mixed barcodes among cells with low amounts of viral mRNA.
...
For the other samples, we simply set a minimum threshold of requiring at least a fraction 2$\times$$10^{-4}$ reads to come from viral mRNA as explained in the legend to \FIGSUPP[viralbarcodes]{thresholds}.
The thresholds for each sample are shown in \FIG{viralbarcodes}C and \FIGSUPP[viralbarcodes]{thresholds}.
...
\jdbcomment{This file is too large for the \textit{eLife} submission system, and so is in the process of being uploaded to DataDryad.}
}
\end{suppfile}
