...
Table delineating genes in (Figure~\ref{fig:cellulargenes}) that are associated with the response to oxidative stress.
...
Cell barcodes associated with 2.5-fold less or more UMI counts were considered to be potential doublets or inefficient/empty reactions, and were excluded from further downstream analyses.
...
The derivative of the resultant curve was calculated, and the threshold set at the minimum value wherein the derivative is less than 0 -- indicating the point below which purity drops indicating a significant contribution of lysis-derived rather than infection-derived reads.
Cells meeting this threshold were defined as influneza-infected, and those failing to do so were excluded from further analysis.
...
Using Monocle?s cell hierarchy function, cells were binned into those with no influenza, and those with reads derived from influeza. 
...
Candidate marker genes for t-distributed stochastic neighbor embedding (tSNE) dimensionality reduction that would allow us to seperate these populations were then determined using the Monocle function markerDiffTable, excluding the effects of sample variation and the number of genes identified in a given cell, using a q-value cutoff of 0.01.
...
Second, as influenza has the capacity to suppress the host transcriptome, and itself ?takes up? UMIs that are attributed to a cell, we decided to assign the cell?s size factor, a scalar value representing efficiency of UMI capture, based on host transcripts alone. 
...
Genes whose expression covaried with the fraction of mRNAs in a cell derived from influenza were then identified, using the Monocle function differentialGeneTest, and removing variance explained by sample to sample variation. 
...
As an additional analysis, we ranked cells within each sample according to the relative percentage of reads derived from influneza.
...
Genes whose expression correlated with influenza ranking were then determined using the Monocel function deifferentialGeneTest, again removing variance explained by sample to sample variation and using a q-value threshold of 0.1. 
