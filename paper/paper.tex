...
In addition to detailed information regarding influenza transcription, we obtained concurrent data describing the host transcriptome, granting insight into the impact of influenza replication on its host.
...
After defining these states, we undertook tSNE dimensionality reduction to determine whether these populations could be distinguished by transcriptome state.
Indeed these populations did appear to be discriminable, indicative of underlying biological variability.
Moreover, uninfected cells group relatively well, regardless of timepoint, suggesting the most important factor driving these differences is influenza infection, and not additional sample variation or paracrine signaling.
...
When we look for cells expressing either type I or type III interferons, we find a single cell expressing both signaling pathways. 
As type I and III interferons are thought to be expressed after activation of MAVS downstream of RIG-I recognition of influneza, their cooccurrence is unsurprising.
Even rare cells expressing interferon could have an outsized effect on ISG expression in the population.
We therefore extended our analysis to examining a prototypical ISG, IFIT1. 
Several cells express higher than baseline levels of IFIT1 (should we look at all datasets and see if more at 10h??), but on the whole there appears to be little paracrine signaling.
...
The lack of interferon production in influenza-infected cells is somewhat surprising given the frequent absence of the major interferon atnagonist, NS1. 
Mutant stains lacking this protein generate high levels of interferon.
Nevertheless, when we examine absence of NS in this dataset, we see that while the single interferon positive cell does lack expression from the NS gene segment, many cells lack NS and yet do not express interferon. 
This is consistent with prior reports that, despite high levels of interferon induction, even NS1 mutants do not induce interferon deterministically. 
...
We then ordered our influenza-infected cells in pseudotime, from few influenza transcripts observed to many, and visualized expression data for these 45 genes with respect to this timecourse.
