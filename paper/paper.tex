...
We then co-infected cells with a mix, of wildtype and $\Delta$HA-GFP virus and used flow cytometry to score cells for the presence of HA only (infection by wildtype virus), GFP only (infection by $\Delta$HA-GFP virus), or both (co-infection) as shown in Figure~\ref{fig:flowcyto}A.
As in our single-cell sequencing data, we found that expression of HA and GFP were highly correlated (Figure~\ref{fig:flowcyto}B), indicating that co-infected cells typically expressed roughly equal amounts of transcript from each viral variant.

\begin{figure}
\centerline{\includegraphics[width=0.9\linewidth]{figures/Pseudovirus_flow_cytometry/coinfFlow_D01.pdf}}
\caption{\label{fig:flowcyto} Co-infected cells express roughly equal amounts of a gene from each infecting viral variant. 
...
{\bf(C)} {\color{red} Get rid of this.} 
{\bf(D)} {\color{red} Include this in supplement for figure.} 
}
\figsupp[Additional replicates and gating controls.\label{figsupp:flowcyto}]{
}{Figure showing additional replicates and gating controls.}
\end{figure}
