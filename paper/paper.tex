...
\subsection{Generation of influenza virus stocks for single-cell infections.}

Schematic of how the experiment works and generation of barcoded virus (Figure~\ref{fig:workflow}).

Validation of viral populations: wildtype and synonymous barcode (we need to pick some consistent nomenclature) grow about the same (Figure~\ref{fig:viruspopulations}A).
Maybe need a sentence or two about defectives should be working in as background before the next sentence (Carolyn Lopes paper on flu, some other stuff).
The ratio of HA molecules to infectious virus particles is much lower than for a high-defective population (Figure~\ref{fig:viruspopulations}B).
These viruses don't induce very much IFN both overall and compared to a high-defective population (Figure~\ref{fig:viruspopulations}C).
The number of HA-expressing infection events is not all that much higher than the number of actual infectious events, in contrast to a high-defective population (Figure~\ref{fig:viruspopulations}D).
...
\begin{figure}
\includegraphics[angle=270,width=0.7\linewidth]{figures/Validating_barcode_virus/validating_populations_D02.pdf}
\caption{
CAPTION}
\label{fig:viruspopulations}
\end{figure}

\subsection{Global analysis of single-cell transcript counts reveals robustness of approach.}
