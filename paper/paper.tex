...
A549 cells were infected at an MOI of {\color{red}??} in either the presence or absence of the protein-translation inhibitor cyclohexamide, and viral mRNA was quantified at 10 hours post-infection by qPCR.
...
The absence of segments appears to be an independent event, as the probability of observing all four non-RNP segments in a cell is well predicted by simply multiplying the probabilities of observing each of the four segments individually (Figure~\ref{fig:fluburdenbyflugene}C). 
...
This and other reports have described the considerable heterogeneity in influenza infection in tissue culture. 
However, in contrast to our analyses above, the fraction of influenza mRNAs derived from a given segment behaves in a stereotyped fashion, with median values consistent with prior population-level measurements.
Rather than the several order of magnitude degree of variance observed in influenza mRNA abundance, the inter-quartile range of influenza ratios observed tends to be relatively tight (add a percentage here).
This trend appears to hold  whether or not cells are expressing mRNAs from all eight segments, at all three time points,  and, in cells running the gamut of infectious burden.
...
This is consistent with more limited experiments showing relatively consistent positive correlations between several segments in vRNA measurements in single cells.
...
When we examine our co-infected cells, we observe a higher rate of expression of all eight segments (metric).
Consistent with this model, we frequently observe a single barcode for a given gene segment, suggesting it was only provided by one virion.
This does, of course, make the assumption that absence is not instead driven by competition between segments.
...
To support these conclusions, we generated a virus in which the HA segment was replaced by GFP, allowing us to detect co-infection by both wild-type and mutant virus by flow cytometry. 
...
Moreover, a portion of the population expressing only low HA, which likely includes those viruses only engaging in primary transcription, is greatly reduced, suggesting rescue of this phenotype by co-infection. 
